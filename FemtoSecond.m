...
plotmin= 1e-4; %%Labeling convention: short-wavelength beam (Pump beam) is order# S_Orders+3 
...
side= (omegaIntervals - 1)/2; %%This now sets the actual resolution, which will be as good as or better than 
...
RegIntervals= floor(points/spacing); 
%%number of intervals over entire Regen range 
...
%%%%%CHECK THIS LINE%%%
RegenSpec(floor(70/spacing): ceil(220/spacing), 2)= RegenSpec(floor(70/spacing): ceil(220/spacing), 2)./scale; 

%%Initialize the Frequency and Intensity arrays 
%%I'm adding a couple extra orders on either end as it's needed later 
omega= (omegaPump - (omegaIntervals*(S_Orders + 2) + side)*omegaRes: omegaRes: omegaPump + (omegaIntervals*(A_Orders + 2) + side)*omegaRes);
...
ngas= 1 + Pressure*0.7e30./((31.9e15)^2 - (2.*pi.*c.*100.*omega).^2); %%from physrevA68_023812p15_2003 
...
for z= 0: Intervals     
	...
	%%  (de-excitation is given by its complex-conjugate)         
		...
		for n= 3*omegaIntervals + 1: (Pump + A_Orders)*omegaIntervals         
			%%Using the definition of Q below, I need to start from one order above the minimum 
			%            Omega= omega(n) - omega(n - omegaIntervals + x);             
			Q= Q + B(n)*conj(B(n - omegaIntervals + x))*exp( -i*(k(n) - k(n - omegaIntervals + x))*dz);
			%                *1/(omegaRaman^2 - Omega^2 - 2*i*Omega*HWHMRaman);         
			end         
		P(z + 1)= real(Q);
		%%Increment each order         
		for n= 2*omegaIntervals + 1: (Pump + A_Orders)*omegaIntervals             
			C(n)= B(n) + g*Intensity/ngas(n)*omega(n)/omega(Pump)*(conj(Q)*B(n + omegaIntervals - x)*exp(i*(k(n) - k(n + omegaIntervals - x))*dz)- Q*B(n - omegaIntervals + x)*exp(i*(k(n) - k(n - omegaIntervals + x))*dz))*dz;
		end
	end
...
SY= (minorder - side/omegaIntervals: 1/omegaIntervals: maxorder + side/omegaIntervals); 
%%See diagram below 
%%    ..|....|....|....|....|....|....|.. 
%%               min   P   max 
